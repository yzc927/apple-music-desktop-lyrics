...
- 管理器中的“全局快捷键”可修改组合键或留空禁用；默认 Ctrl+Alt+H 显示/隐藏、Ctrl+Alt+L 锁定/解锁、Ctrl+Alt+左右箭头调整偏移、Ctrl+Alt+R 循环当前句。保存时检查重复及系统占用，未能注册的动作会在管理器提示。隐藏到托盘时仍可使用。
- “查看候选 / 排除错版”展示专辑、时长差、词级时间轴标记和前两句预览。选中后可使用或排除；排除记忆按歌曲保存，支持恢复。歌曲切换后需重新打开列表，避免误操作另一首歌。
- “本曲读音 / 生词注音”支持选择或输入词语，保存自定义读音及熟词标记；解锁歌词后也可双击注音打开。开启“仅显示生词注音”后隐藏手动标记的熟词，不会推测用户词汇水平。纠正和熟词状态按歌曲保存，可恢复自动读音。
- 上述设置仅存于本机 `interaction-preferences.json`，不会上传词语或学习记录。
