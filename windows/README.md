...
其虚拟化的可见 `Line` 队列；LRCLIB 模式严格使用本句到下一句的真实时间戳间隔，不再根据字数猜测时长。
同一句的填色进度只允许前进，媒体进度的小幅抖动不会让颜色来回晃动。
扫色会在换句前稍早完成；媒体时钟采样偶尔跨过句尾时，也会先短暂显示完整旧句，避免半截扫色直接消失。
卡拉 OK 扫色以约 30 帧刷新；高字密度快句会自动采用最多约 25% 的自适应压缩，普通句和慢句
仍使用原始时间轴。LRCLIB 行异常停滞时，仅在 Apple 当前句和下一句能高置信度匹配到后方歌词后
才自动恢复同步。
